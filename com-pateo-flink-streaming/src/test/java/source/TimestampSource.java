...
public class TimestampSource extends RichSourceFunction<DataPoint<Long>>
		implements Checkpointed<Long> {

	private static final long serialVersionUID = -7731752144227769618L;
	...
	public void run(SourceContext<DataPoint<Long>> ctx) throws Exception {
		while (running) {
			synchronized (ctx.getCheckpointLock()) {
				ctx.collectWithTimestamp(new DataPoint<>(currentTimeMs, 0L),
						currentTimeMs);
